...
library(igraph)
#Create Modules
## Enrichment map on genesets database
# declare pathways database
gmxFile <- "data/c2.cp.v7.2.symbols.gmt"
...
# write results
#outNet <- outNet[outNet$counts<1500,]
#write.table(outNet, file = "PathwayModules-770.txt", sep = "\t", quote = F, row.names = F)
...
#outNet <- read.csv("PathwayModules-v1.txt", sep="\t", stringsAsFactors = F)
################################################################################
